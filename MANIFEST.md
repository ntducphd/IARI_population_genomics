...
| [2b] | `03b_umap_structure.py` **[built]** | 3rd triangulation method, run entirely in Python for a dependency-independent path (DAPC is fit separately in R at stage 21): PLINK --recode A -> mean-impute -> UMAP(2D) → `tables/umap_{set}.csv` |
...
| [3b] | `04b_nj_tree.R` **[built, ran]** | NJ tree from the IBS distance (ape, run as its own process for dependency isolation) → `interim/nj_{set}.nwk`, `tables/nj_{set}_tips.csv`. Bootstrap deferred (STRETCH: IQ-TREE). |
| [4] | `05_fst_amova.py` **[built, ran]** | Pairwise + global Fst between admixture clusters (Nei's Gst Hs/Ht, computed directly in Python) + permutation test (999 perms) on the LD-pruned set → `tables/fst_pairwise_{set}.csv`, `fst_global_{set}.csv`. Real: Set1 global Fst=0.330 (p=0.001), Set2 global Fst=0.294 (p=0.001), pairwise range 0.05-0.39, comparable to Zhao et al. 2011's 0.23-0.53 across 5 rice subpopulations. |
...
| [23] | 23_diversity_recompute.py | pi/theta_W/Tajima D on missingness-only SNPs (avoids MAF-filter ascertainment bias) | diversity_nomaf_summary, diversity_nomaf_windows_* |
...
| [35] | 35_figures_composite.py | High-density composite mains for the final 10-figure scheme: Fig02_structure (6p), Fig04_diff_diversity_ld (4p), Fig05_concordance_mechanism (6p), Fig08_application (3p), Fig09_mechanism_visible (6p), Fig10_platforms (3p). | figures/main/Fig{02,04,05,08,09,10}_*.png+pdf |
...
## Robustness and validation stages (30-33)

| Stage | Script | Role | Key outputs |
|---|---|---|---|
| [30] | 30_confound_robustness.py | Stature-confound Mantel battery, position-proxy tests, height-only/feature-subset classifier baselines, external-3K-RGP-label classifier, K+/-1 targets, soft-Q accuracy, 999-perm nulls, bootstrap CIs | confound_mantel_*, spatial_proxy_*, classifier_baselines_*, external_label_classifier_set1, mantel_bootstrap_* |
| [31] | 31_demography_repair.py | Stairway summary cleaned (Ne columns, underflow rows), within-largest-cluster Tajima D + LD-Ne, Ne interval across F-source x map-density grid | stairway_ne_set1 (clean), within_cluster_history_set1, ne_interval_summary |
| [32] | 32_overlap_null.py | Circular-rotation null for two-scan region overlap + exact-SNP overlap; pcadapt saturation + per-chromosome SNP density bookkeeping | overlap_null_summary, pcadapt_saturation, snp_density_by_chrom |
| [33] | 33_validate_reference.py | Bespoke Mantel vs vegan::mantel; Procrustes sensitivity recomputed with the stage-09 input pipeline (reconciles the M2 band) | mantel_vegan_validation, robustness_procrustes_* (overwritten) |
