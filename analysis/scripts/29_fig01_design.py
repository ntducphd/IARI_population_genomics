...
FS, FS_ST, FS_SUP, FS_LEG = 13.0, 13.5, 18.0, 11.5   # sized for half-page embedding (author feedback 2026-08-09)


def _wrap2(text, limit=40):
    """Split a detail line near its middle at ' · ' / '; ' / ', ' / ' ' so large fonts stay
    inside the fixed-width boxes without dropping content."""
    if len(text) <= limit:
        return [text]
    for sep in (" · ", "; ", ", ", " "):
        positions = [i for i in range(len(text)) if text.startswith(sep, i)]
        if positions:
            cut = min(positions, key=lambda i: abs(i - len(text) // 2))
            left = text[:cut].rstrip()
            right = text[cut + len(sep):].lstrip()
            if sep.strip() == "·" and not right.startswith("·"):
                right = "· " + right
            return [left, right]
    return [text]
...
def box(x, y, w, h, title, detail="", fc=NEUT, ec=EDGE, tc=INK, lw=2.0):
    ax.add_patch(FancyBboxPatch((x, y), w, h, boxstyle=BS, fc=fc, ec=ec, lw=lw, zorder=3))
    limit = max(24, int(w * 1.15))
    if detail:
        lines = _wrap2(detail, limit)
        if len(lines) == 1:
            ax.text(x + w / 2, y + h * 0.66, title, ha="center", va="center",
                    fontsize=FS, fontweight="bold", color=tc, zorder=4)
            ax.text(x + w / 2, y + h * 0.28, lines[0], ha="center", va="center",
                    fontsize=FS, color="#2A2A2A", zorder=4)
        else:
            ax.text(x + w / 2, y + h * 0.76, title, ha="center", va="center",
                    fontsize=FS, fontweight="bold", color=tc, zorder=4)
            ax.text(x + w / 2, y + h * 0.44, lines[0], ha="center", va="center",
                    fontsize=FS, color="#2A2A2A", zorder=4)
            ax.text(x + w / 2, y + h * 0.16, lines[1], ha="center", va="center",
                    fontsize=FS, color="#2A2A2A", zorder=4)
    else:
...
def arrow(x1, y1, x2, y2, c=EDGE, lw=2.2, sb=1.6):
    ax.add_patch(FancyArrowPatch((x1, y1), (x2, y2), arrowstyle="-|>", mutation_scale=19,
                                 lw=lw, color=c, zorder=2, shrinkA=1.2, shrinkB=sb))


def stage(n, y, t, cx=3.2, tx=6.0):
    ax.add_patch(Circle((cx, y), 1.9, fc=EDGE, ec="none", zorder=5))
    ax.text(cx, y, str(n), ha="center", va="center", fontsize=12.5, color="white",
...
for i, (lab, c) in enumerate([("Set 1 (WGS)", S1C), ("Set 2 (50K array)", S2C),
                              ("Phenomics", PHC)]):
    x0 = 61 + i * 21
    ax.add_patch(FancyBboxPatch((x0, 71.4), 3.2, 2.0, boxstyle=BS, fc=tint(c), ec=c,
                                lw=1.8, zorder=5))
...
box(C3X[0], 55.6, C3W, 6.8, "Set 1 — WGS",
    "n = 150 · 1.01M → 502,675 QC → 24,370 pruned", ec=S1C, fc=tint(S1C, 0.16))
box(C3X[1], 55.6, C3W, 6.8, "Set 2 — 50K array",
    "n = 147 · 50,051 → 31,565 QC → 2,942 pruned", ec=S2C, fc=tint(S2C, 0.16))
box(C3X[2], 55.6, C3W, 6.8, "Canopy imaging + traits",
    "204 features × 3 timepoints · agronomic / NUE traits", ec=PHC, fc=tint(PHC, 0.16))
for xc in C3C:
...
YB = 53.3
ax.plot([38, 38, 60.9], [55.6, YB, YB], color=S1C, lw=2.2, zorder=2,
        solid_capstyle="round", solid_joinstyle="round")
ax.plot([63.1, C3C[2]], [YB, YB], color=S1C, lw=2.2, zorder=2,
        solid_capstyle="round", solid_joinstyle="round")
...
box(24, 21.0, 76, 7.2, "Does canopy imaging carry genetic ancestry?",
    ...
    ec=PHC, fc=tint(PHC, 0.32), lw=2.0)
...
ax.plot([115, 121, 121], [59, 59, 24.6], color=PHC, lw=2.2, zorder=2,
        solid_capstyle="round", solid_joinstyle="round")
